...
nodes(1)=start_node;
figure(1)
plot(start_node.pos(1),start_node.pos(2),'O','Color',[0 0.036 0.33]);
hold on
plot(goal_node.pos(1),goal_node.pos(2),'O','Color',[0 0.036 0.33]);
hold on
for i=1:no_of_nodes
    disp(i);
    ...
    random_node = get_random_points(goal_node,x_max,y_max);
    ...
    plot(new_node.pos(1),new_node.pos(2),'.','Color',[255 0 0]/255);
...
for j=1:length(nodes)
        ...
        D=[D tmp];
        plot(nodes(j).pos(1),nodes(j).pos(2),'.','Color',[255 0 0]/255);
        hold on
...
function random_node=get_random_points(goal_node,x_max,y_max)
   if randi(100)>15
     random_node=[rand(1)*x_max,rand(1)*y_max];
   else
...
function new_node=create_new_node(random_node,near_node,near_node_value,near_node_index,max_dist)
    ...
    new_node.pos= near_node.pos + max_dist.*[cos(theta) sin(theta)];
...
function nodes=rewire_step_2(neighbour_node_index,new_node,nodes)
    for j=1:length(neighbour_node_index)
        ...
        if rewire_cost < nodes(neighbour_node_index(j)).cost
            ...
            line([nodes(neighbour_node_index(j)).pos(1),nodes(parent_node_ind).pos(1)],...
            [nodes(neighbour_node_index(j)).pos(2),nodes(parent_node_ind).pos(2)],...
